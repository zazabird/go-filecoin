...
func (c *Expected) validateMining(ctx context.Context,
	...
	for i := 0; i < ts.Len(); i++ {
		...
		fmt.Printf("[TICKET VALIDATION] cid = %s\n", blk.Cid())
		fmt.Printf("[TICKET VALIDATION] newPeriod=%t, sampleEpoch=%d, blk.Miner=%s, workerSigner=%s, blk.Ticket=%x\n", newPeriod, sampleEpoch, blk.Miner, workerSignerAddr, blk.Ticket)
